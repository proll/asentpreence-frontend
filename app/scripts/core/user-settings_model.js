aspe.UserSettings = Backbone.Model.extend({
	url: "/api/v2/auth/me/",
	...
	initialize: function () {
		aspe.on('social:update', function(social_obj) {
			var social = this.get('social') || {};
			this.set('social', _.extend(social, social_obj));
		}, this);
	},
	...
	success: function (response, status, xhr) {
		...
		if (!!response.error){
			...
			aspe.error('UserSettings', response.error)
			this.trigger("error", {description: "WHP/auth : check state ERROR after netcall!"})
		} else {
			this.set(response);
			this.trigger("usersettings:ready", response)
		}
	},

	error: function(xhr, status, err){
		aspe.error('UserSettings', status);
	...
	getGeoError: function (position_error) {
		console.warn(position_error.message);
		// aspe.error('UserSettings', position_error.message);
	}
});